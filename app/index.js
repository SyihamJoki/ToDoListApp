...
export default function HomeScreen() {
  ...
  const [openItems, setOpenItems] = useState({}); // State untuk menyimpan status open/close setiap item
  ...
  const toggleItem = (id) => {
    setOpenItems((prevOpenItems) => ({
      ...prevOpenItems,
      [id]: !prevOpenItems[id], // Membalik status open/close berdasarkan id
    }));
  };
  ...
  const handleComplete = async (id, currentStatus) => {
    try {
      ...
      await updateDoc(docRef, {
        completed: !currentStatus, // Ubah status completed
      });
      ...
      {/* Floating Action Button */}
        <TouchableOpacity
            style={[
                ...
                zIndex: 1, // Menambahkan zIndex agar tombol berada di atas komponen lain
                position: 'absolute', // Pastikan tombol diposisikan di atas
                },
            ]}
            onPress={() => {
          ...
          dataList.map((item) => (
            <View key={item.id} style={[{ width: windowWidth * 0.9, alignItems: 'center', marginBottom:5 }]}>
              ...
              <TouchableOpacity
                ...
                onPress={() => toggleItem(item.id)} // Toggle item on press
              >
                ...
                <View style={[styles.collapsibleContent, { width: windowWidth * 0.85 }]}>
                  ...
                  <View style={{flexDirection:'row', gap:5, marginTop:10}}>
                    <TouchableOpacity
                        ...
                        onPress={() => handleDelete(item.id)} // Fungsi hapus
                    >
                    ...
                    <TouchableOpacity
                        style={[styles.buttonItem, { width: windowWidth * 0.4, height: windowHeight * 0.05, paddingStart: windowWidth * 0.1 }]}
                        onPress={() => handleComplete(item.id, item.completed)} // Fungsi selesai
                    >
